...
const setupNetwork = async () => {
    ...
    log(
        "Please connect to a wired or wireless network (via iwctl) and rerun the script.",
    );
    process.exit(1);
};
...
const installNeovimConfig = async () => {
    if (existsSync(NVIM_CONFIG_DIR)) {
        if (existsSync(`${NVIM_CONFIG_DIR}/.git`)) {
            try {
                const remoteUrl =
                    await $`git -C ${NVIM_CONFIG_DIR} remote get-url origin`.text();
                if (remoteUrl.includes("knownasnaffy/nvim")) {
                    log(
                        "Neovim configuration already exists and is from the correct repository. Skipping...",
                    );
                    return;
                }
                log(
                    "Neovim configuration exists but is from a different repository. Removing it...",
                );
            } catch {
                log(
                    "Neovim configuration exists but is not a Git repository. Removing it...",
                );
            }
        } else {
            log(
...
const installPrograms = async () => {
    ...
    await $`paru -Sy --noconfirm beeper-v4-bin hyprhalt hyprquickframe-git quicksnip-git`;
...
const installPlugins = async () => {
    ...
    if (existsSync(spaceshipPath)) {
        ...
        await createSymlink(
            `${spaceshipPath}/spaceship.zsh-theme`,
...
const linkDotfiles = async () => {
    ...
    await createSymlink(
        `${DOTFILES_DIR}/.config/ghostty`,
        `${HOME}/.config/ghostty`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.config/qutebrowser`,
        `${HOME}/.config/qutebrowser`,
    );
    await createSymlink(`${DOTFILES_DIR}/.config/rofi`, `${HOME}/.config/rofi`);
    await createSymlink(
        `${DOTFILES_DIR}/.config/fastfetch`,
    ...
    await createSymlink(
        `${DOTFILES_DIR}/.config/zathura`,
        `${HOME}/.config/zathura`,
    );
    await createSymlink(`${DOTFILES_DIR}/.config/yazi`, `${HOME}/.config/yazi`);
    await createSymlink(
        `${DOTFILES_DIR}/.config/pycodestyle`,
        `${HOME}/.config/pycodestyle`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.config/systemd`,
        `${HOME}/.config/systemd`,
    );
    ...
    await createSymlink(
        `${DOTFILES_DIR}/.config/xdg-desktop-portal-termfilechooser`,
        `${HOME}/.config/xdg-desktop-portal-termfilechooser`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.config/bluetuith`,
        `${HOME}/.config/bluetuith`,
    );
    await createSymlink(`${DOTFILES_DIR}/.config/mpv`, `${HOME}/.config/mpv`);
    await createSymlink(
        `${DOTFILES_DIR}/.config/swayosd`,
        `${HOME}/.config/swayosd`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.config/swayimg`,
        `${HOME}/.config/swayimg`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.config/quickshell`,
    ...
    await createSymlink(
        `${DOTFILES_DIR}/.config/BeeperTexts/custom.css`,
        `${HOME}/.config/BeeperTexts/custom.css`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.config/BeeperTexts/config.json`,
        `${HOME}/.config/BeeperTexts/config.json`,
    );

    await createSymlink(
        `${DOTFILES_DIR}/.local/bin/qutebrowser.sh`,
        `${HOME}/.local/bin/qutebrowser`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.local/bin/crypt.sh`,
        `${HOME}/.local/bin/crypt`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.local/bin/git-status.sh`,
        `${HOME}/.local/bin/git-status`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.local/bin/start-hotspot.sh`,
        `${HOME}/.local/bin/start-hotspot`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.local/bin/stop-hotspot.sh`,
        `${HOME}/.local/bin/stop-hotspot`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/.local/bin/slurp.sh`,
        `${HOME}/.local/bin/slurp`,
    );

    await createSymlink(
        `${DOTFILES_DIR}/.local/share/fonts`,
        `${HOME}/.local/share/fonts`,
    );

    await createSymlink(
        `${DOTFILES_DIR}/media/pictures/fastfetch.png`,
        `${HOME}/Pictures/fastfetch.png`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/media/pictures/wallpaper-mask-layer.png`,
        `${HOME}/Pictures/wallpaper-mask-layer.png`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/media/pictures/wallpaper-background-layer.jpg`,
        `${HOME}/Pictures/wallpaper-background-layer.jpg`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/media/music/notification-1.mp3`,
        `${HOME}/Music/notification-1.mp3`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/media/music/notification-2.mp3`,
        `${HOME}/Music/notification-2.mp3`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/media/music/windows-connected.mp3`,
        `${HOME}/Music/windows-connected.mp3`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/media/music/windows-disconnected.mp3`,
        `${HOME}/Music/windows-disconnected.mp3`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/media/music/battery-warning.mp3`,
        `${HOME}/Music/battery-warning.mp3`,
    );
    await createSymlink(
        `${DOTFILES_DIR}/media/music/battery-critical.mp3`,
        `${HOME}/Music/battery-critical.mp3`,
    );

    await createSudoSymlink(
        `${DOTFILES_DIR}/etc/ly/config.ini`,
        "/etc/ly/config.ini",
    );
