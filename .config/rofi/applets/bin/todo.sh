#!/bin/bash
#
# this script manages a folder of todo lists using the modi in rofi-todo.sh
#
# dependencies: rofi

themes_dir="$HOME/.config/rofi/launchers/type-1"
dmenu_theme="$themes_dir/dmenu.rasi"
text_input_theme="$themes_dir/text-input.rasi"

SCRIPT_PATH="$( cd -- "$(dirname "$0")" >/dev/null 2>&1 || exit; pwd -P )"

ROFI="${ROFI:-rofi}"
DATA_DIR="$HOME/notes"

todo_dir="$DATA_DIR/todo"

mkdir -p "$todo_dir"

get_todo_lists() {
    cd "$todo_dir" && find * -type f | xargs -I{} wc -l {}
}

while todo_file=$(echo -en "New ToDo List\n$(get_todo_lists)" |\
        $ROFI -dmenu -i -p "ToDo List" -theme "$dmenu_theme"); do

    if [[ "$todo_file" = "New ToDo List"* ]]; then
        todo_file=$( $ROFI -dmenu -p "List Name" -theme "$text_input_theme" )
    else
        todo_file=$(echo "$todo_file" | cut -d' ' -f2-) # remove items count
    fi

    [ -z "$todo_file" ] && exit 1

    TODO_FILE="$todo_dir/$todo_file" \
        $ROFI -modi "ToDo $todo_file:$SCRIPT_PATH/todo-helper.sh" -show "ToDo $todo_file" -theme "$dmenu_theme"
done
